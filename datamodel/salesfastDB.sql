...
INSERT INTO `alignments` VALUES (1,1,1,1,1,'11001'),(2,2,1,1,1,'11001'),(3,3,2,2,1,'11003');
...
CREATE TABLE `meeting_experience` (
  ...
  `isPhysicianEntry` binary(1) NOT NULL,
  `isSalesRepEntry` binary(1) NOT NULL,
  `status` varchar(45) NOT NULL,
  `genuinelyLikedTheProduct` binary(1) NOT NULL,
  `genuinelyDidnotLikedTheProduct` binary(1) NOT NULL,
  `affordablePrice` binary(1) NOT NULL,
  `notAffordablePrice` binary(1) NOT NULL,
  `lessSideEffects` binary(1) NOT NULL,
  `moreSideEffects` binary(1) NOT NULL,
  `likedPresentation` binary(1) NOT NULL,
  `didnotLikedPresentation` binary(1) NOT NULL,
  `salesRepConfident` binary(1) NOT NULL,
  `salesRepNotConfident` binary(1) NOT NULL,
  `companyReputationImpressive` binary(1) NOT NULL,
  `companyReputationNotImpressive` binary(1) NOT NULL,
  `Meeting_Update_meetingUpdateId` int(11) NOT NULL,
  PRIMARY KEY (`meetingExperienceId`),
  KEY `fk_Meeting_Experience_Meeting_Update1_idx` (`Meeting_Update_meetingUpdateId`),
  CONSTRAINT `fk_Meeting_Experience_Meeting_Update1` FOREIGN KEY (`Meeting_Update_meetingUpdateId`) REFERENCES `meeting_update` (`meetingUpdateId`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
